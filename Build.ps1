param(
    [ValidateSet('Verify', 'Runtime', 'Kernel', 'BootTest', 'RuntimeTest')][string]$Mode = 'Verify',
    [string]$Zig = '',
    [ValidateSet('none', 'poweroff', 'reboot', 'ram')][string]$BootProbe = 'none',
...
try {
    ...
    if ($Mode -ne 'Verify') {
        ...
        $runtimeArguments = @()
        if ($BootProbe -in @('none', 'ram') -or $Mode -eq 'Runtime') {
