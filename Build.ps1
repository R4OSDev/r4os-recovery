param(
    [ValidateSet('Verify', 'Runtime', 'Kernel', 'BootTest', 'RuntimeTest', 'StorageTest', 'StorageAccessTest', 'StorageToolsTest', 'InputTest', 'UITest', 'NetworkTest')][string]$Mode = 'Verify',
    [string]$Zig = '',
...
try {
    ...
    if ($Mode -ne 'Verify') {
        ...
        if ($Mode -in @('NetworkTest', 'StorageAccessTest', 'StorageToolsTest')) { $BootProbe = 'none' }
        if (!$Zig) {
        ...
        if ($Mode -in @('NetworkTest', 'StorageAccessTest', 'StorageToolsTest')) {
            & pwsh -NoLogo -NoProfile -File (Join-Path $PSScriptRoot 'Tools/Test-Network.ps1') -Firmware $Firmware -Zig $Zig -StorageAccess:($Mode -eq 'StorageAccessTest') -StorageTools:($Mode -eq 'StorageToolsTest') -TimeoutSeconds 300
