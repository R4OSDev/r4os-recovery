...
pub fn build(b: *std.Build) void {
    const RecoveryProbe = enum { none, poweroff, reboot, ram, storage, input, ui };
    ...
    if ((recovery_probe == .none or recovery_probe == .ram or recovery_probe == .storage or recovery_probe == .input or recovery_probe == .ui) and (runtime_sha256.len != 64 or runtime_bytes == 0))
