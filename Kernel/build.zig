...
pub fn build(b: *std.Build) void {
    const RecoveryProbe = enum { none, poweroff, reboot, ram, storage, input };
    ...
    if ((recovery_probe == .none or recovery_probe == .ram or recovery_probe == .storage or recovery_probe == .input) and (runtime_sha256.len != 64 or runtime_bytes == 0))
