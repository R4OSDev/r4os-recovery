...
const Codec = struct {
    pub fn inspect(_: Codec, bytes: []const u8, entries: []r4os.zip.Entry) !r4os.zip.Info { return core.inspect(bytes, entries); }
    pub fn begin(_: Codec, bytes: []const u8, entry: *const r4os.zip.Entry, output: []u8, work: *r4os.zip.Work) !r4os.zip.Progress { return core.begin(bytes, entry.*, output, &work.data); }
    pub fn step(_: Codec, work: *r4os.zip.Work, budget: u32) !r4os.zip.Progress { return core.step(&work.data, budget); }
};
...
pub fn main(init: std.process.Init) !void {
    ...
    if (args.len == 5 and std.mem.eql(u8, args[1], "--write-plan")) {
        ...
        const plan = try slots.Plan.prepare(a, image[2363392 * 512 ..][0 .. 512 * 1024 * 1024], 2363392,
            state.guid.parse(identity.value.installationId) orelse return error.InvalidTarget, source, false, .{});
    ...
    for ([_]bool{ false, true }) |current_phase| {
        for ([_]Fault{ .second_write, .first_flush, .readback }) |fault| {
            ...
            const errors = switch (fault) { .second_write => error.WriteFailed, .first_flush => error.FlushFailed, .readback => error.VerifyFailed, else => unreachable };
