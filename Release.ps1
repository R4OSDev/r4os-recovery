param([Parameter(Position=0)][ValidateSet('Prepare','Publish','SelfTest')][string]$Action='Prepare',[switch]$Technical,[switch]$Prerelease,[switch]$Prepared)
$ErrorActionPreference='Stop';Set-StrictMode -Version Latest
$requestedAction=$Action;$requestedPrerelease=[bool]$Prerelease;$requestedPrepared=[bool]$Prepared;$root=$PSScriptRoot;$workspace=[IO.Path]::GetFullPath((Join-Path $root '../..'))
try {
 . (Join-Path $root 'Tools/Package.ps1')
 . (Join-Path $root 'Tools/PackagePair.ps1')
 . (Join-Path $root 'Tools/Inventory.ps1')
 . (Join-Path $root 'Tools/PreparedRelease.ps1')
 ...
 if($requestedPrepared -and $requestedAction -ine 'Publish'){throw '-Prepared is only valid for Publish.'}
 ...
 if($requestedAction -ceq 'SelfTest'){
  ...
  Test-PreparedRecoveryAssets (Join-Path $workspace 'Temp/recovery-prepared-release-selftest')
 ...
 if($requestedPrepared){
  $releasePreparation=Get-PreparedRecoveryRelease $root $version $kernelVersion $commit
 }else{
 ...
 Write-Host "Recovery assets ready: $($releasePreparation.OutputRoot) (technical=$Technical)"
 exit 0
